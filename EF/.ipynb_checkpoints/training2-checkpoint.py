...
class MessagePassingModel(nn.Module):
    ...
    def EFD(self, atomic_numbers, positions, Ef, dst_idx_flat, src_idx_flat, batch_segments, batch_size, dst_idx=None, src_idx=None):
        ...
        Ef_flat = jnp.repeat(Ef, N, axis=0) 
        
        # We choose Feature Index 0 to be our "Non-Equivariant" channel.
        # In e3x tensor format (B*N, parity, (lmax+1)^2, features):
        # Component indices 1, 2, 3 correspond to l=1 (x, y, z).
        
        # We manually overwrite Feature 0, Parity 0 (even), l=1 components with raw Ef values.
        # This breaks equivariance because these values won't transform with the grid.
        xEF = xEF.at[:, 0, 1:4, 0].set(Ef_flat) #### NON-EQUIVARIENT
        #xEF = e3x.nn.change_max_degree_or_type(xEF, max_degree=0,
